...
struct Node {
    ...
    ~Node() { cout << "Node with value " << value << " destructor" <<endl; }
};
...
public:
    ...
    void insertFirstNode(shared_ptr<Node<T>> nodePtr);
...
template<class T>
void BidirectionalList<T>::insertFirstNode(shared_ptr<Node<T>> nodePtr) {
    head->next = nodePtr;
    nodePtr->prev = head;
    tail->prev = nodePtr;
    nodePtr->next = tail;
...
void BidirectionalList<T>::pushBack(T item) {
    shared_ptr<Node<T>> nodePtr = make_shared<Node<T>>(item);
    if (isEmpty()) {
        insertFirstNode(nodePtr);
        return;
    }
...
bool BidirectionalList<T>::popFront() {
    ...
    if (head->next == tail) {
        tail->prev = weak_ptr<Node<T>>();
        head->next = shared_ptr<Node<T>>();
    }
    return true;
}
...
bool BidirectionalList<T>::popBack() {
   ...
   shared_ptr<Node<T>> tail_prev = tail->prev.lock();
...
void BidirectionalList<T>::print() {
   ...
   shared_ptr<Node<T>> it = head->next;
